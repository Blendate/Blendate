...
extension InterestsView {
    ...
    private func creasteUserDoc() {
        do {
            sessionState.user.filters.gender = getSeeking()
            try createDoc(from: sessionState.user)
    ...
    func getSeeking()->Gender?{
        guard let gender = sessionState.user.filters.gender else {return nil}
        switch gender {
        case .male:
            return .female
        case .female:
