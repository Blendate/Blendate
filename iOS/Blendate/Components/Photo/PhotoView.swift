...
//    @State var photoData: Data?
    ...
    var body: some View {
        ...
        .errorAlert(error: $error) { alert in
            Button("Try again"){ uploadImage(photoItem) }
    ...
    struct Error: ErrorAlert {
        var title: String = "Image Error"
        var message: String = "There was an error uploading your photo, please try again or use a different photo"
    }
}
...
struct PhotoView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        PhotoView(url: alice.photos[0]!.url, shape: Circle())
        PhotoView(alice.photos[0]!)
